...
class Robot:
    ...
    def convertState(self):
        return torch.cat((torch.tensor([self.table.turn]),torch.tensor(self.table.table).view(-1))).float().to("cuda")
    ...
    def sampling(self, n, pi, error, frominitial=True, ifshow = False):
        ...
        for i in range(n):
            ...
            if winning == 0:
                ...
                if ifnormal == False:
                    if len(error) < 10000:
                        error.append((torch.cat((torch.tensor([state0[0]]).view(-1).float(),torch.tensor(state0[1]).view(-1).float())).to("cuda"), torch.tensor(action).view(-1).float(), reward))
                    else:
                        error.pop(0)
                        error.append((torch.cat((torch.tensor([state0[0]]).view(-1).float(),torch.tensor(state0[1]).view(-1).float())).to("cuda"), torch.tensor(action).view(-1).float(), reward))
                    continue
...
class Trainer:
    def __init__(self, policy, qnet, qnetcopy, robot, gamma=0.9, device="cuda"):
        self.policy = policy.to(device)
        self.qnet = qnet.to(device)
        self.qnetcopy = qnetcopy.to(device)
        self.qnet.train()
        self.qnetcopy.train()
        self.robot = robot
        self.gamma = gamma
    ...
    def updaterebuff(self, sample):
        state0 = torch.cat((torch.tensor([sample[0][0]]),torch.tensor(sample[0][1]).view(-1))).float().to("cuda")
        state1 = torch.cat((torch.tensor([sample[3][0]]),torch.tensor(sample[3][1]).view(-1))).float().to("cuda")

        action = torch.tensor(sample[1]).view(-1).float().to("cuda")
        reward = torch.tensor(sample[2]).float().to("cuda")
